...
abstract class MockupData {
    ...
    private static Dialog getDialog(int i, Date lastMessageCreatedAt) {
        ArrayList<MockUser> users = getUsers();
        MockUser mUs = users.get(0);
        User mockUs = new User(mUs.getId(), "asdas@gmail.com", mUs.getName(), "Uysal", "5555555", false);
        ArrayList<User> olArtık = new ArrayList<>();
        olArtık.add(mockUs);
        ArrayList<Message> messagesss =  new ArrayList<Message>();
        messagesss.add(getMessage(lastMessageCreatedAt));
        return new Dialog(
                getRandomId(),
                getRandomAvatar(),
    ...
    private static ArrayList<MockUser> getUsers() {
    ...
    private static MockUser getUser() {
        return new MockUser(
                getRandomId(),
                getRandomName(),
                getRandomAvatar());
